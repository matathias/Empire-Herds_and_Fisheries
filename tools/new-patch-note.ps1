...
# Run from the Empire-AnimalHusbandry/ directory (the one containing About/, 1.6/, etc.)
...
if (-not (Test-Path $defsDir) -or -not (Test-Path $aboutPath)) {
    Write-Host "ERROR: Run this script from the Empire-AnimalHusbandry/ directory (the one containing About/ and 1.6/)." -ForegroundColor Red
    exit 1
}
...
$defName = "AH_${major}_${minor}_${patch}"
...
${indent}<FactionColonies.PatchNoteDef ParentName="AHPatchBase">
...
$csprojPath = "1.6\Source\Core\Empire.AnimalHusbandry.csproj"
if (Test-Path $csprojPath) {
    ...
    Write-Host "  Empire.AnimalHusbandry.csproj AssemblyVersion/FileVersion updated to $assemblyVersion" -ForegroundColor Cyan
} else {
